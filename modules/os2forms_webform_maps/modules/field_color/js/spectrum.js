...
(function (factory) {
  ...
  var defaultOpts = {
      ...
      className: "", // Deprecated - use containerClassName and
                     // replacerClassName instead.
      containerClassName: "",
      replacerClassName: "",
      showAlpha: true,
      theme: "sp-light",
    ...
    markup = (function () {
      ...
      // IE does not support gradients with multiple stops, so we need to
      // simulate that for the rainbow slider with 8 divs that each have a
      // single gradient
      var gradientFix = "";
      if (IE) {
        for (var i = 1; i <= 6; i++) {
  ...
  function spectrum(element, o) {
    ...
    // Element to be updated with the input color. Populated in initialize
    // method
    ...
    //If there is a label for this element, when clicked on, show the colour
    // picker
    ...
    function initialize() {
      ...
      // Prevent clicks from bubbling up to document.  This would cause it to
      // be hidden.
    ...
    function updateSelectionPaletteFromStorage() {
      ...
      if (localStorageKey) {
        // Migrate old palettes over to new format.  May want to remove this
        // eventually.
        try {
    ...
    function updateUI() {
      ...
      // Update dragger background color (gradients take care of saturation and
      // value).
      var flatColor = tinycolor.fromRatio({h: currentHue, s: 1, v: 1});
      dragger.css("background-color", flatColor.toHexString());

      // Get a format that alpha will be included in (hex and names ignore
      // alpha)
      ...
      if (!realColor && allowEmpty) {
        // Update the replaced elements background with icon indicating no
        // color selection
        ...
        // Update the replaced elements background color (with actual selected
        // color)
    ...
    function updateHelperLocations() {
      ...
      if (allowEmpty && isEmpty) {
        ...
        // Where to show the little circle in that displays your current
        // selected color
    ...
    function updateOriginalInput(fireCallback) {
      ...
      if (fireCallback && hasChanged) {
        ...
        // we trigger the change event or input, but the input change event is
        // also binded to some spectrum processing, that we do no need
    ...
    function reflow() {
      if (!visible) {
        return; // Calculations would be useless and wouldn't be reliable
                // anyways
      }
  ...
  /**
   * checkOffset - get the offset below/above and left/right element depending
   * on screen position Thanks
  ...
  /**
   * stopPropagation - makes the code only doing this a little easier to read
   * in line
   */
   ...
   * when dragging, the x is within [0,element.width] and y is within
   * [0,element.height]
   */
  ...
  $.fn.spectrum.inputTypeColorSupport = function inputTypeColorSupport() {
    if (typeof inputTypeColorSupport._cachedResult === "undefined") {
      var colorInput = $("<input type='color'/>")[0]; // if color element is
                                                      // supported, value will
                                                      // default to not null
  ...
  (function () {
    ...
    var tinycolor = function (color, opts) {
      ...
      // Potentially lose a little bit of precision here, but will fix issues
      // where .5 gets interpreted as half of the total, instead of half of 1
      // If it was supposed to be 128, this was already taken care of by
      // `inputToRgb`
      if (this._r < 1) {
    ...
    tinycolor.prototype = {
      ...
      toHsvString: function () {
        var hsv = rgbToHsv(this._r, this._g, this._b);
        var h = mathRound(hsv.h * 360), s = mathRound(hsv.s * 100),
      ...
      toHslString: function () {
        var hsl = rgbToHsl(this._r, this._g, this._b);
        var h = mathRound(hsl.h * 360), s = mathRound(hsl.s * 100),
    ...
    // contained [0, 1] or [0, 100] *Returns:* { r, g, b } in the set [0, 255]
    ...
    // contained in [0, 1] or [0, 100] *Returns:* { r, g, b } in the set [0,
    // 255]
    ...
    // Spin takes a positive or negative amount within [-360, 360] indicating
    // the change of hue. Values outside of this range will be wrapped into
    // this range.
    ...
    // sufficient contrast. *Example* tinycolor.isReadable("#000", "#111") =>
    // false
    ...
    var matchers = (function () {
      ...
      // Allow positive/negative integer/number.  Don't capture the either/or,
      // just the entire outcome.
    ...
    // `stringInputToObject`
    // Permissive string parsing.  Take in a number of formats, and output an
    // object based on detected format.  Returns `{ r, g, b }` or `{ h, s, l }`
    // or `{ h, s, v}`
    function stringInputToObject(color) {
      ...
      // Keep most of the number bounding out of this function - don't worry
      // about [0,1] or [0,100] or [0,360] Just return an object and let the
      // conversion functions handle that. This way the result will be the same
      // whether the tinycolor is initialized with string or object.
      var match;
